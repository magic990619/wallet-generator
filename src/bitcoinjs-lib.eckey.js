...
Bitcoin.ECKey = (function () {
	...
	*/
	ECKey.decodeWalletImportFormat = function (privStr) {
		...
		var hash = bytes.slice(0, bytes.length - 4);
		var checksum = Crypto.SHA256(Crypto.SHA256(hash, { asBytes: true }), { asBytes: true });
		if (checksum[0] != bytes[bytes.length - 4] ||
					checksum[1] != bytes[bytes.length - 3] ||
					checksum[2] != bytes[bytes.length - 2] ||
					checksum[3] != bytes[bytes.length - 1]) {
	...
	*/
	ECKey.decodeCompressedWalletImportFormat = function (privStr) {
		...
		if (janin.selectedCurrency.CWIF_Start !== ""){
			hash.pop();
		}
